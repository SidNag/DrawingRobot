...
placeG = [0.15,-0.05,0.028]
placeY = [0.15,0.40,0.028]
placeGup = [0.15,-0.05,0.028+0.03]
placeYup = [0.15,0.40,0.028+0.03]
...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    start = lab_invk(*start_xw_yw_zw, 0)
    end = lab_invk(*target_xw_yw_zw, 0)
    startAbove = lab_invk(start_xw_yw_zw[0], start_xw_yw_zw[1], start_xw_yw_zw[2] + 0.07,0)
    endAbove = lab_invk(target_xw_yw_zw[0], target_xw_yw_zw[1], target_xw_yw_zw[2] + 0.07,0)    

    move_arm(pub_cmd, loop_rate, startAbove, vel, accel)
    move_arm(pub_cmd, loop_rate, start, vel, accel)
    ...
    move_arm(pub_cmd, loop_rate, startAbove, vel, accel)
    move_arm(pub_cmd, loop_rate, endAbove, vel,accel)
    move_arm(pub_cmd, loop_rate, end, vel,accel)
    gripper(pub_cmd, loop_rate, 0)
    time.sleep(0.5)
    move_arm(pub_cmd, loop_rate, endAbove, vel,accel)
...
def main():
    ...
    if len(xw_yw_G) == 2:
        G1 = np.array([xw_yw_G[0][0], xw_yw_G[0][1], 0.028])
        G2 = np.array([xw_yw_G[1][0], xw_yw_G[1][1], 0.028])
        move_block(pub_command, loop_rate, G1, placeG, vel, accel)
        move_block(pub_command, loop_rate, G2, placeGup, vel, accel)
    ...
    if len(xw_yw_Y) == 2:
        Y1 = np.array([xw_yw_Y[0][0], xw_yw_Y[0][1], 0.028])
        Y2 = np.array([xw_yw_Y[1][0], xw_yw_Y[1][1], 0.028])
        move_block(pub_command, loop_rate, Y1, placeY, vel, accel)
        move_block(pub_command, loop_rate, Y2, placeYup, vel, accel)
    elif len(xw_yw_Y) == 1:
        Y1 = np.array([xw_yw_Y[0][0], xw_yw_Y[0][1], 0.028])
        move_block(pub_command, loop_rate, Y1, placeY, vel, accel)
